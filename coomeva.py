...
def app():
    st.header('SEGUIMIENTO DE PQR - COOMEVA')
#     st.subheader("Listado de PQR")
    ...
    df['eps']=df['eps'].str.replace('COOMEVA EPS','COOMEVA')
    dfcoomeva=df[(df['eps'] == "COOMEVA")]
    full=len(dfcoomeva)
    st.write("Al día de hoy, Medicontrol SAS ha recibido en total ",full," PQRs enviadas directamente por la EPS Coomeva, las mismas han sido solicitadas con el fin de hacer seguimiento y vigilancia al comportamiento resolutivo de cada uno de las Peticiones, Quejas o Reclamos de los usuarios por partes de la EAPB.")
    ...
    dfcoomeva['fechainc'] = pd.to_datetime(dfcoomeva['fechainc'], format='%Y-%m-%d')
    mask = (dfcoomeva['fechainc'] > start_date) & (dfcoomeva['fechainc'] <= end_date)
    dfcoomeva = dfcoomeva.loc[mask]
    
    dfver=dfcoomeva[(dfcoomeva['fecha'] != "0000-00-00")]
    total=len(dfcoomeva)
    ver=len(dfver)
 
    st.write(" En el tiempo comprendido entre las fechas ",start_date," y ",end_date,", fueron recibidas ", total," PQR correspondientes a Coomeva EPS. De estas han sido verificadas ", ver," a través de medios sincrónicos no presenciales. Y a continuación se muestra la información analizada a partir de dicha verificación.")
    ...
    with col1:
        ...
        st.write("Aqui puede observar el histograma de radicación de PQR de la EPS Coomeva durante el periodo seleccionado.")
