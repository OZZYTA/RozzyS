...
#     st.subheader("Listado de PQR")
    ...
    dfnuevaeps=df[(df['eps'] == "NUEVA EPS")]
    full=len(dfnuevaeps)
    st.write("Al día de hoy, Medicontrol SAS ha recibido en total ",full," PQRs enviadas directamente por la Nueva EPS, las mismas han sido solicitadas con el fin de hacer seguimiento y vigilancia al comportamiento resolutivo de cada uno de las Peticiones, Quejas o Reclamos de los usuarios por partes de la EAPB.")
    ...
    dfnuevaeps['fechainc'] = pd.to_datetime(dfnuevaeps['fechainc'], format='%Y-%m-%d')
    mask = (dfnuevaeps['fechainc'] > start_date) & (dfnuevaeps['fechainc'] <= end_date)
    dfnuevaeps = dfnuevaeps.loc[mask]
    
    dfver=dfnuevaeps[(dfnuevaeps['fecha'] != "0000-00-00")]
    total=len(dfnuevaeps)
    ver=len(dfver)
 
    st.write(" En el tiempo comprendido entre las fechas ",start_date," y ",end_date,", fueron recibidas ", total," PQR correspondientes a Nueva EPS. De estas han sido verificadas ", ver," a través de medios sincrónicos no presenciales. Y a continuación se muestra la información analizada a partir de dicha verificación.")
    ...
    with col1:
        ...
        st.write("Aqui puede observar el histograma de radicación de PQR de la EPS Nueva EPS durante el periodo seleccionado.")
