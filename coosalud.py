...
def app():
    st.header('SEGUIMIENTO DE PQR - COOSALUD')
#     st.subheader("Listado de PQR")
    ...
    df['eps']=df['eps'].str.replace('COOSALUD EPS','COOSALUD')
    dfcoosalud=df[(df['eps'] == "COOSALUD")]
    full=len(dfcoosalud)
    st.write("Al día de hoy, Medicontrol SAS ha recibido en total ",full," PQRs enviadas directamente por las EPS Coosalud, las mismas han sido solicitadas con el fin de hacer seguimiento y vigilancia al comportamiento resolutivo de cada uno de las Peticiones, Quejas o Reclamos de los usuarios por partes de la EAPB.")
    ...
    dfcoosalud['fechainc'] = pd.to_datetime(dfcoosalud['fechainc'], format='%Y-%m-%d')
    mask = (dfcoosalud['fechainc'] > start_date) & (dfcoosalud['fechainc'] <= end_date)
    dfcoosalud = dfcoosalud.loc[mask]
    
    dfver=dfcoosalud[(dfcoosalud['fecha'] != "0000-00-00")]
    total=len(dfcoosalud)
    ver=len(dfver)
 
    st.write(" En el tiempo comprendido entre las fechas ",start_date," y ",end_date,", fueron recibidas ", total," PQR correspondientes a Coosalud EPS. De estas han sido verificadas ", ver," a través de medios sincrónicos no presenciales. Y a continuación se muestra la información analizada a partir de dicha verificación.")
    ...
    with col1:
        ...
        st.write("Aqui puede observar el histograma de radicación de PQR de la EPS Coosalud durante el periodo seleccionado.")
