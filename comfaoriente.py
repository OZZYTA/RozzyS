...
#     st.subheader("Listado de PQR")
    ...
    st.write("Al día de hoy, Medicontrol SAS ha recibido en total ",full," PQRs enviadas directamente por las EPS Comfaoriente, las mismas han sido solicitadas con el fin de hacer seguimiento y vigilancia al comportamiento resolutivo de cada uno de las Peticiones, Quejas o Reclamos de los usuarios por partes de la EAPB.")
    ...
    dfcomfaoriente['fechainc'] = pd.to_datetime(dfcomfaoriente['fechainc'], format='%Y-%m-%d')
    mask = (dfcomfaoriente['fechainc'] > start_date) & (dfcomfaoriente['fechainc'] <= end_date)
    ...
    st.write(" En el tiempo comprendido entre las fechas ",start_date," y ",end_date,", fueron recibidas ", total," PQR correspondientes a Comfaoriente EPS. De estas han sido verificadas ", ver," a través de medios sincrónicos no presenciales. Y a continuación se muestra la información analizada a partir de dicha verificación.")
    ...
    with col1:
        ...
        st.write("Aqui puede observar el histograma de radicación de PQR de la EPS Comfaoriente durante el periodo seleccionado.")
