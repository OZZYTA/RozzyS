...
def app():
    st.header('SEGUIMIENTO DE PQR - COMPARTA')
#     st.subheader("Listado de PQR")
    ...
    dfcomparta['fechainc'] = pd.to_datetime(dfcomparta['fechainc'], format='%Y-%m-%d')
    mask = (dfcomparta['fechainc'] > start_date) & (dfcomparta['fechainc'] <= end_date)
    dfcomparta = dfcomparta.loc[mask]
    
    dfver=dfcomparta[(dfcomparta['fecha'] != "0000-00-00")]
    total=len(dfcomparta)
    ver=len(dfver)
 
    st.write(" En el tiempo comprendido entre las fechas ",start_date," y ",end_date,", fueron recibidas ", total," PQR correspondientes a Comparta EPS. De estas han sido verificadas ", ver," a través de medios sincrónicos no presenciales. Y a continuación se muestra la información analizada a partir de dicha verificación.")
    ...
    with col1:
        ...
        st.write("Aqui puede observar el histograma de radicación de PQR de la EPS Comparta durante el periodo seleccionado.")
