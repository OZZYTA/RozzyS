...
#     st.subheader("Listado de PQR")
    ...
    dfmedimas['fechainc'] = pd.to_datetime(dfmedimas['fechainc'], format='%Y-%m-%d')
    mask = (dfmedimas['fechainc'] > start_date) & (dfmedimas['fechainc'] <= end_date)
    dfmedimas = dfmedimas.loc[mask]
    
    dfver=dfmedimas[(dfmedimas['fecha'] != "0000-00-00")]
    total=len(dfmedimas)
    ver=len(dfver)
 
    st.write(" En el tiempo comprendido entre las fechas ",start_date," y ",end_date,", fueron recibidas ", total," PQR correspondientes a Medimas EPS. De estas han sido verificadas ", ver," a través de medios sincrónicos no presenciales. Y a continuación se muestra la información analizada a partir de dicha verificación.")
    ...
    with col1:
        ...
        st.write("Aqui puede observar el histograma de radicación de PQR de la EPS Medimas durante el periodo seleccionado.")
