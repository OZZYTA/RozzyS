...
def app():
    st.header('SEGUIMIENTO DE PQR - SANITAS')
#     st.subheader("Listado de PQR")
    ...
    df['eps']=df['eps'].str.replace('SANITAS EPS','SANITAS')
    dfsanitas=df[(df['eps'] == "SANITAS")]
    full=len(dfsanitas)
    st.write("Al día de hoy, Medicontrol SAS ha recibido en total ",full," PQRs enviadas directamente por la EPS Sanitas, las mismas han sido solicitadas con el fin de hacer seguimiento y vigilancia al comportamiento resolutivo de cada uno de las Peticiones, Quejas o Reclamos de los usuarios por partes de la EAPB.")
    ...
    dfsanitas['fechainc'] = pd.to_datetime(dfsanitas['fechainc'], format='%Y-%m-%d')
    mask = (dfsanitas['fechainc'] > start_date) & (dfsanitas['fechainc'] <= end_date)
    dfsanitas = dfsanitas.loc[mask]
    
    dfver=dfsanitas[(dfsanitas['fecha'] != "0000-00-00")]
    total=len(dfsanitas)
    ver=len(dfver)
 
    st.write(" En el tiempo comprendido entre las fechas ",start_date," y ",end_date,", fueron recibidas ", total," PQR correspondientes a Sanitas EPS. De estas han sido verificadas ", ver," a través de medios sincrónicos no presenciales. Y a continuación se muestra la información analizada a partir de dicha verificación.")
    ...
    with col1:
        ...
        st.write("Aqui puede observar el histograma de radicación de PQR de la EPS Sanitas durante el periodo seleccionado.")
