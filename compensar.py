...
def app():
    st.header('SEGUIMIENTO DE PQR - COMPENSAR')
#     st.subheader("Listado de PQR")
    ...
    df['eps']=df['eps'].str.replace('COMPENSAR EPS','COMPENSAR')
    dfcompensar=df[(df['eps'] == "COMPENSAR")]
    full=len(dfcompensar)
    st.write("Al día de hoy, Medicontrol SAS ha recibido en total ",full," PQRs enviadas directamente por las EPS Compensar, las mismas han sido solicitadas con el fin de hacer seguimiento y vigilancia al comportamiento resolutivo de cada uno de las Peticiones, Quejas o Reclamos de los usuarios por partes de la EAPB.")
    ...
    dfcompensar['fechainc'] = pd.to_datetime(dfcompensar['fechainc'], format='%Y-%m-%d')
    mask = (dfcompensar['fechainc'] > start_date) & (dfcompensar['fechainc'] <= end_date)
    dfcompensar = dfcompensar.loc[mask]
    
    dfver=dfcompensar[(dfcompensar['fecha'] != "0000-00-00")]
    total=len(dfcompensar)
    ver=len(dfver)
 
    st.write(" En el tiempo comprendido entre las fechas ",start_date," y ",end_date,", fueron recibidas ", total," PQR correspondientes a Compensar EPS. De estas han sido verificadas ", ver," a través de medios sincrónicos no presenciales. Y a continuación se muestra la información analizada a partir de dicha verificación.")
    ...
    with col1:
        ...
        st.write("Aqui puede observar el histograma de radicación de PQR de la EPS Compensar durante el periodo seleccionado.")
